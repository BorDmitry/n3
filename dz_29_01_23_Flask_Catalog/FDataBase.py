...
class FDataBase:
    ...
    def add_auto(self, title, text):
        try:
            self.__cur.execute("SELECT COUNT() as 'count' FROM auto WHERE text LIKE ?", (text,))
            res = self.__cur.fetchone()
            if res['count'] > 0:
                print("Статья с таким text уже существует")
                return False
            tm = math.floor(time.time())
            self.__cur.execute("INSERT INTO auto VALUES(NULL, ?, ?, ?)", (title, text, tm))
            self.__db.commit()
    ...
    def get_auto_anonce(self):
        try:
            self.__cur.execute("SELECT id, title, text FROM auto ORDER BY time DESC")
            res = self.__cur.fetchall()
            if res:
                return res
        except sqlite3.Error as e:
