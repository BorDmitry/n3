...
class FDataBase:
    def __init__(self, db):
        self.__db = db           # открыли соединение с базой данных и
        self.__cur = db.cursor()  # создали объект курсора

    def get_menu(self):
        sql = "SELECT * FROM mainmenu"
        try:
            self.__cur.execute(sql)    # попробуем выполнить запрос sql( execute()- считывает запрос)
            res = self.__cur.fetchall()  # сохраняем все результаты в пер res
            if res:
                return res  # выводим данные в консоль, если они есть
        except IOError:
